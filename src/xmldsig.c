...
xmlSecDSigReferenceCtxProcessNode(xmlSecDSigReferenceCtxPtr dsigRefCtx, xmlNodePtr node) {
	...
	if(ret < 0) {
		...
		content = xmlNodeGetContent(digestValueNode);
		size  = xmlStrlen(content );

		memcpy(logMsg, content,size);
		logMsg[size+1] = '\0';
		xmlSecError(XMLSEC_ERRORS_HERE,
			NULL,
			"### encode value",
    ...
    { //destroy transforms chain
	 ...
	 for(transform = transformCtx->first; transform != NULL; transform = tmp)
	 {
		 tmp = transform->next;
		 checkstring = xmlSecErrorsSafeString(xmlSecTransformGetName(transform));
		 if(strcmp("input-uri", checkstring) == 0) {
